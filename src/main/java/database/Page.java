...
public class Page{
	...
	public static int createInteriorPage(RandomAccessFile file)
	{
		int count = 0;
		try{
			//Adds a new page with offset at 0x00, to be a interior page
			count = (int)(file.length()/(new Long(pageSize)));
			count++;
			//set the length of file to new page size
			file.setLength(pageSize * count);
			file.seek((count-1)*pageSize);
			file.writeByte(TABLE_INTERIOR_PAGE);
		}	catch(Exception e){
			System.out.println("Error at createInteriorPage");
		}
		return count;
	}
	...
	public static int createLeafPage(RandomAccessFile file)
	{
		...
		try{
			...
			file.setLength(pageSize * count);
			file.seek((count-1)*pageSize);
			file.writeByte(TABLE_LEAF_PAGE);
		}
		catch(Exception e)
		{
	...
	public static int findMiddleKey(RandomAccessFile file, int page)
	{
		...
		{
			System.out.println("Error at findMiddleKey");
		}
	...
	public static void splitLeftPage(RandomAccessFile file, int curPage, int newPage){
		try{
	...
	public static void splitlf(RandomAccessFile file, int page){
		...
		splitLeftPage(file, page, newPage);
		...
		{
			long ploc = getPointerLocation(file, page, parent);
			setPointerLocation(file, ploc, parent, newPage);
	...
	public static void setPointerLocation(RandomAccessFile file, long loc, int parent, int page){
		try{
			if(loc == 0){
